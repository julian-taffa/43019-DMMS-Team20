...
#include "DFRobot_I2CMultiplexer.h"           // i2c multiplexer
...
int numOfSensors = 8;  // the number of sensors per strip

int numOfStrips = 1;  // the number of strips
...
void setup() {
  ...
  sensorData = new SensorData*[numOfStrips];  // allocate an array of SensorData pointers
  i2cMultiplexers = new DFRobot_I2CMultiplexer[numOfStrips];

  // the following code allocates the arrays based upon the input variables of the number of strips and number of sensors per strip
  for (int i = 0; i < numOfStrips; i++) {
    sensorData[i] = new SensorData[numOfSensors];  // allocate an array of SensorData for each strip
    for (int j = 0; j < numOfSensors; j++) {
      sensorData[i][j].readings = new float[numOfSensors];  // allocate an array of floats for each SensorData
    }
  }
  ...
  for (int i = 0; i < numOfStrips; i++) {
    if (!i2cMultiplexers[i].begin(Wire1)) {
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Multiplexer");
      lcd.setCursor(0, 1);  // Sets cursor to 2nd row, 1st character
      lcd.print("No.");
      lcd.print(i);
      lcd.print(" Failed");
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Initialising Sensors");
  lcd.setCursor(0, 1);  // Sets cursor to 2nd row, 1st character
  lcd.print("Please wait...");
